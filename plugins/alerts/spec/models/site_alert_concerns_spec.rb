    ...
    describe "when hit threshold" do
      describe "send email and sms to all selected users_field" do 
        ...
        it "should add sms_que into Resque.enqueue" do 
          SmsTask.should have_queued([user.phone_number], threshold.message_notification, 'smart', collection.id).in(:sms_queue_lite)
        end

        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued([user.email], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end 
      ...
      describe "send email and sms to all selected fields" do
        ...
        it "should add sms_que into Resque.enqueue" do 
          SmsTask.should have_queued(['123456'], threshold.message_notification, 'smart', collection.id).in(:sms_queue_lite)
        end

        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued(['foo@example.com'], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end
      ...
      describe "send email and sms to all selected fields, members and users" do
        ...
        it "should add sms_que into Resque.enqueue" do 
          SmsTask.should have_queued([user.phone_number, '123456', user_2.phone_number], threshold.message_notification, 'smart', collection.id).in(:sms_queue_lite)
        end

        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued([user.email, 'foo@example.com', user_2.email], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end
    end
  end
  ...
  describe "edit site" do
    describe "when hit threshold" do
      describe "send email and sms to all selected users_field" do 
        ...
        it "should add sms_que into Resque.enqueue" do 
          SmsTask.should have_queued([user_2.phone_number], threshold.message_notification, 'smart', collection.id).in(:sms_queue_lite)
        end

        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued([user_2.email], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end 
      ...
      describe "send email and sms to all selected fields" do
        ...
        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued([user_3.email], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end
      ...
      describe "send email and sms to all selected fields, members and users" do
        ...
        it "should add sms_que into Resque.enqueue" do 
          SmsTask.should have_queued([user.phone_number, user_2.phone_number, user_3.phone_number], threshold.message_notification, 'smart', collection.id).in(:sms_queue_lite)
        end

        it "should add email_que into Resque.enqueue" do 
          EmailTask.should have_queued([user.email, user_2.email, user_3.email], threshold.message_notification, "[ResourceMap] Alert Notification").in(:email_queue_lite)
        end
      end
    end
  end
