class Ability
  ...
  def initialize(user, format = nil)
    ...
    # Admin abilities
    can [:destroy, :create_snapshot, :recreate_index, :update, :members,:send_new_member_sms, :register_gateways, :export, :print_template, :copy], Collection, :memberships => { :user_id => user.id , :admin => true }
    ...
    can :read, Layer, :collection => {:layer_memberships => { :user_id => user.id , :read => true }}
    can :read, Layer, :collection => {:layer_memberships => { :user_id => user.id , :write => true }}

    # Member Abilities
    can [:csv_template, :upload_csv, :unload_current_snapshot, :load_snapshot, :register_gateways, :message_quota, :reminders, :settings, :quotas, :upload_members, :print_template, :copy], Collection, :memberships => { :user_id => user.id }
