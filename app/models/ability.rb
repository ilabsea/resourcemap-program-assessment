class Ability
  ...
  def initialize(user)
    ...
    # Admin abilities
    can [:destroy, :create_snapshot, :recreate_index, :update, :members,:send_new_member_sms, :register_gateways, :print_template], Collection, :memberships => { :user_id => user.id , :admin => true }
    ...
    if !user.is_guest
      can [:new, :create, :query], Collection
    end
