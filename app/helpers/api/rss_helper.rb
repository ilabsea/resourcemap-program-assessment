module Api::RssHelper
  ...
  def site_item_rss(xml, result)
    ...
    xml.item do
      ...
      xml.pubDate Site.parse_time(source['updated_at']).rfc822 if source['updated_at'].present?
      xml.startEntryDate Site.parse_time(source['start_entry_date']).rfc822 if source['start_entry_date'].present?
      xml.endEntryDate Site.parse_time(source['end_entry_date']).rfc822 if source['end_entry_date'].present?
