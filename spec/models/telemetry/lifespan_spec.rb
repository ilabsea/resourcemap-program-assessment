...
describe Telemetry::Lifespan do
  ...
  it 'updates the collection lifespan' do
    ...
    InsteddTelemetry.should_receive(:timespan_update).with('collection_lifespan', {collection_id: collection.id}, collection.created_at, @now)
  ...
  it 'updates the collection users lifespan' do
    ...
    Telemetry::Lifespan.should_receive(:touch_user).with(user1).at_least(:once)
    Telemetry::Lifespan.should_receive(:touch_user).with(user2).at_least(:once)
